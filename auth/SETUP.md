...
El mismo lab implementado en 3 stacks para comparar frameworks, todos con
el SSO (OIDC) real de cada nube y organizados con **screaming
architecture**: cada uno tiene carpetas/paquetes `aws/`, `azure/`, `gcp/`
en la raiz, en vez de un controlador generico por `{cloud}`.
...
por los 3 stacks (mismas credenciales, distintas redirect URIs por puerto).
...
- Docker + Docker Compose (los 3 stacks corren en contenedor, no hace falta Python/Node/Go instalados en local)
...
Los 3 stacks usan la misma convencion, `/auth/{cloud}/callback`. Hay que
dar de alta las 3 URLs de cada nube en el proveedor correspondiente (o
dejar que Terraform lo haga por ti para AWS/Azure, ver arriba):

| Nube | Python (8000) | NestJS (8003) | Go (8005) |
|---|---|---|---|
| AWS Cognito | `.../auth/aws/callback` | `.../auth/aws/callback` | `.../auth/aws/callback` |
| Azure AD | `.../auth/azure/callback` | `.../auth/azure/callback` | `.../auth/azure/callback` |
| Google | `.../auth/gcp/callback` | `.../auth/gcp/callback` | `.../auth/gcp/callback` |

(sustituye `...` por `http://localhost:PUERTO` segun la columna, ej.
`http://localhost:8005/auth/aws/callback` para Go).

## 3. AWS Cognito (SSO para `/aws`)

**Opcion rapida:** `cd ../terraform && make apply && make outputs` crea el
User Pool + App Client con las 3 callback URLs ya configuradas.
...
3. En **App integration**:
   ...
   - En el App client, configura:
     - **Allowed callback URLs**: las 3 URLs de AWS Cognito de la tabla anterior (una por linea).
     - **Allowed sign-out URLs**: `http://localhost:PUERTO/aws` para cada uno de los 3 puertos.
     - **OAuth grant types**: Authorization code grant
     - **OpenID scopes**: `openid`, `email`, `profile`
4. Anota: region, User pool ID, Client ID, Client secret y el dominio del Hosted UI.
5. Rellena en `.env` (en `auth/`, compartido por los 3 stacks):
   ```
   AWS_COGNITO_REGION=...
...
2. Redirect URIs (tipo **Web**): agrega las 3 URLs de Azure AD de la tabla anterior.
...
4. **Authorized redirect URIs**: agrega las 3 URLs de Google de la tabla anterior.
...
make up      # docker compose build + up -d de los 3 stacks
```
...
Para levantar/parar un stack suelto en vez de los 3, usa `STACK=`:
```bash
make up STACK=go
make down STACK=go
make logs STACK=go
...
- Los 3 stacks comparten el mismo `.env`: mismas credenciales de cada nube, cada uno solo usa su propia `BASE_URL`/puerto para construir la redirect URI.
- NestJS usa `openid-client`, que valida `client_id` no vacio al **construir** el cliente (no solo al hacer login). Por eso su `.env` usa un placeholder ("unset") cuando una nube no esta configurada, en vez de dejar el valor vacio. Python y Go no tienen ese problema: construyen la peticion al vuelo o al arrancar sin validar contra el proveedor.
